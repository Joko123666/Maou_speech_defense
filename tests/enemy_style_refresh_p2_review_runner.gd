...
const OUTPUT_PATH := "res://tests/visual_reviews/enemy_style_refresh_p2.png"
const ASSET_IDS := [
	&"civilian_slime",
	&"jiane_succubus_bewitcher",
	&"judaginda_cult_applicant",
]
...
func _build_review() -> void:
	...
	page.add_child(_label("v0.19 ENEMY STYLE REFRESH / P2", Color("f0d58a"), 22))
	page.add_child(_label("1254px RGBA · direct left-facing art · mobile role checks at 220 / 96 / 64 / 48 / 36px density", Color("aeb8cf"), 12))
...
func _asset_column(content_id: StringName) -> PanelContainer:
	...
	stack.add_child(_texture_rect(_texture(content_id), 220))
	stack.add_child(_label("ENEMY-FACING ← · DIRECT ART", Color("f0d58a"), 10))

	var sizes := HBoxContainer.new()
	sizes.alignment = BoxContainer.ALIGNMENT_CENTER
	sizes.add_theme_constant_override(&"separation", 16)
	...
	stack.add_child(_label("8-UNIT DENSITY · 36PX", Color("8f9ab2"), 9))
	var density := HBoxContainer.new()
	density.alignment = BoxContainer.ALIGNMENT_CENTER
	density.add_theme_constant_override(&"separation", 2)
	for index in 8:
		density.add_child(_texture_rect(_texture(content_id), 36))
	stack.add_child(density)
	return panel

func _texture(content_id: StringName) -> Texture2D:
	return load("res://assets/graphics/style_refresh_v019/enemy_p2/%s.png" % content_id) as Texture2D
...
func _capture_and_quit() -> void:
	...
	if error == OK:
		print("ENEMY P2 STYLE REVIEW CAPTURE PASS: %s" % OUTPUT_PATH)
	else:
		push_error("enemy P2 style review capture failed with error %d" % error)
	get_tree().quit(error)
