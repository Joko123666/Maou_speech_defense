...
const REVIEW_OUTPUT_A := "res://tests/visual_reviews/character_style_refresh_defenders_a.png"
const REVIEW_OUTPUT_B := "res://tests/visual_reviews/character_style_refresh_defenders_b.png"
const PAGES := [
	{
		"id": "A",
		"entries": [
...
func _build_review_catalog(page_data: Dictionary) -> void:
	...
	var title := Label.new()
	title.text = "v0.19 DEFENDER FULL-BODY M2-A / PAGE %s — CURRENT · STAGED · 96/64/48 PX" % String(page_data.get("id", ""))
	title.add_theme_font_size_override(&"font_size", 22)
	title.add_theme_color_override(&"font_color", Color("f0d58a"))
	page.add_child(title)
	var note := Label.new()
	note.text = "Isolated review catalog. These staged masters are not referenced by the active GameAssetCatalogData."
...
func _build_defender_column(entry: Dictionary) -> Control:
	...
	var current_path := "res://assets/graphics/defenders/%s.png" % content_id
	var staged_path := "res://assets/graphics/style_refresh_v019/defenders/%s.png" % content_id
	...
	comparison.add_theme_constant_override(&"separation", 6)
	stack.add_child(comparison)
	comparison.add_child(_build_master_sample("CURRENT", current_path, Color("8f9ab2")))
	comparison.add_child(_build_master_sample("STAGED", staged_path, Color("f0d58a")))
	...
	for target_size in [96, 64, 48]:
		...
		sample.add_child(_texture_box(staged_path, Vector2(target_size, target_size)))
...
func _build_master_sample(caption: String, path: String, color: Color) -> Control:
	var stack := VBoxContainer.new()
	stack.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	stack.add_child(_label(caption, color))
	stack.add_child(_texture_box(path, Vector2(126, 220)))
	return stack
...
func _capture_and_quit(output_path: String) -> void:
	var error := await _capture_page(output_path)
	if error == OK:
		print("CHARACTER DEFENDER REVIEW CAPTURE PASS: %s" % output_path)
	else:
		push_error("character defender review capture failed with error %d" % error)
	get_tree().quit(error)

func _capture_page(output_path: String) -> Error:
