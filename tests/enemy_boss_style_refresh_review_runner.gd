...
const OUTPUT_PATH := "res://tests/visual_reviews/enemy_boss_style_refresh.png"
const ASSET_IDS := [
	&"boss_5",
	&"boss_10",
	&"boss_15",
...
func _build_review() -> void:
	...
	page.add_child(_label("v0.19 ENEMY BOSS STYLE REFRESH", Color("f0d58a"), 22))
	page.add_child(_label("1254px RGBA · 7% safe margin · boss readability at 220 / 96 / 64 / 48px · five dedicated identities", Color("aeb8cf"), 12))
...
func _asset_column(content_id: StringName) -> PanelContainer:
	...
	panel.custom_minimum_size = Vector2(235, 0)
	...
	stack.add_child(_label(String(content_id).to_upper(), Color("8f9ab2"), 10))
	stack.add_child(_label(String(DISPLAY_NAMES.get(content_id, content_id)), Color("f7f2e7"), 13))
	...
	for size in [96, 64, 48]:
		...
		sample.add_child(_label("%d" % size, Color("8f9ab2"), 9))
...
func _texture(content_id: StringName) -> Texture2D:
	return load("res://assets/graphics/style_refresh_v019/enemy_bosses/%s.png" % content_id) as Texture2D
...
func _capture_and_quit() -> void:
	...
	if error == OK:
		print("ENEMY BOSS STYLE REVIEW CAPTURE PASS: %s" % OUTPUT_PATH)
	else:
		push_error("enemy boss style review capture failed with error %d" % error)
	get_tree().quit(error)
